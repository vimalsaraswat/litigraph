...
impl WorkspaceCatalog {
    ...
    pub fn add_with_limit(&mut self, workspace: WorkspaceEntry, max_entries: usize) {
        self.add(workspace);
    ...
    pub fn prune(&mut self, max_entries: usize) {
        if self.workspaces.len() > max_entries {
            self.workspaces.truncate(max_entries);
        }
    }
...
mod tests {
    ...
    fn test_catalog_remove_and_find() {
        ...
        assert!(catalog.find_by_path(Path::new("/path/test")).is_some());
    ...
    #[test]
    fn test_catalog_remove_by_path() {
        let mut catalog = WorkspaceCatalog::default();
        let ws = Workspace::new("Test Project");
        let path = PathBuf::from("/path/to/remove");
        let entry = WorkspaceEntry::new(&ws, path.clone());

        catalog.add(entry);
        assert_eq!(catalog.len(), 1);

        catalog.remove_by_path(&path);
    ...
    #[test]
    fn test_catalog_prune_and_add_with_limit() {
        let mut catalog = WorkspaceCatalog::default();
        for i in 0..5 {
            let ws = Workspace::new(format!("WS {}", i));
            catalog.add_with_limit(
                WorkspaceEntry::new(&ws, PathBuf::from(format!("/{}", i))),
                3,
            );
        }

        assert_eq!(catalog.len(), 3);
        assert_eq!(catalog.entries()[0].name, "WS 4");
        assert_eq!(catalog.entries()[1].name, "WS 3");
        assert_eq!(catalog.entries()[2].name, "WS 2");
    }
}
